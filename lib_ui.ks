...
global ui_refTime	is time:seconds. // Reference time (for shorter printouts, use 0 if you want original full time info)
...
function uiConsole {
	...
	if prefix <> "" set prefix to prefix+": ".
	local logtext is round(time:seconds-ui_refTime):toString():padLeft(4) + " " + prefix + msg.
	print logtext.

	if logconsole {
	//	use full time in logs
		log round(time:seconds):toString():padLeft(7) + " " + prefix + msg to "log.txt".
		if homeConnection:isConnected copyPath("log.txt","0:/logs/"+ship:name+".txt").
	}
}
uiConsole("lib_ui", "T=" + round(ui_refTime)).
...
FUNCTION uiBeep {
	...
	set vBeep:volume to 0.2.
	set vBeep:wave to "SINE".
	vBeep:PLAY(NOTE("C4",0.04)).
}
