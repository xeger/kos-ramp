...
SET ok:ONCLICK TO HideGui@. // This could also be an anonymous function instead.
...
//PID Aileron  
SET AileronPID to PIDLOOP(0.01,0.003,0.008). // Kp, Ki, Kd
...
SET RWYKSC_SHUTTLE to latlng(-0.04807,-74.8).
...
until SHIP:STATUS = "LANDED" {
    ...
    IF APATEnabled {
        ...
        IF APMODE = "SHD" {
            ...
            SET VNAVMODE TO "PIT".
            SET TGTPitch TO -8.
            IF SHIP:Altitude > Glideslope(TGTRunway) {
                SET VNAVMODE TO "ALT".
                SET APMODE TO "ILS".
            }
        }
        ...
        ELSE IF APMODE = "ILS" {
            SET TargetCoord TO TGTRunway.
            //Checks if below GS
            SET GSALT to Glideslope(TGTRunway,GSAng).
            SET VNAVMODE TO "ALT".
            IF ship:altitude < GSALT { 
                IF ILSHOLDALT <> TGTAltitude{
                    SET ILSHOLDALT TO SHIP:Altitude.
                    SET TGTAltitude TO ILSHOLDALT.
                }
            }
            ELSE {
                SET TGTAltitude to GSALT.
            }
        ...
        ELSE IF APMODE = "FLR" {
            ...
            IF RadarAltimeter() > 20 {
                SET TGTPitch to PitchAnglePID:UPDATE(TIME:SECONDS,SHIP:VERTICALSPEED + 1).
                IF SHIP:AIRSPEED > 95 {
                    IF NOT BRAKES { BRAKES ON. }
                }
                ELSE {
        ...
        IF APMODE <> "OFF" {
            ...
            ELSE IF VNAVMODE = "SPU" {
                SET ElevatorPID:SETPOINT to MAX(-1,ElevatorPID:SETPOINT - 1) .
            }
        ...
        IF ElevatorPID:MAXOUTPUT <> CTRLIMIT * 1.3 .{
            SET ElevatorPID:MAXOUTPUT TO CTRLIMIT * 1.3 .
            SET ElevatorPID:MINOUTPUT TO -CTRLIMIT * 1.3 .
        }
